...
(function () {
  ...
  /* ---------- PDF download (faithful capture of the on-screen roadmap) -----
     The user loves the in-app look (pixel font, serif headlines, bordered
     cards, two-column inputs), so the PDF is a high-resolution capture of the
     real, fully-styled roadmap DOM via html2canvas — not a re-draw. To stay
     crisp and never cut a card, the worlds are grouped two-per-A4-page (worlds
     1 & 2 on page one, 3 & 4 on page two) and each group is captured on its own
     branded page. No footer or page numbers — clean, like the app. Both
     libraries are only fetched the first time the button is clicked. */
  ...
  /* Build an off-screen "page" that mirrors the on-screen dialog: the branded
     header bar (minus the close button) above a cream body holding the given
     report nodes. Returns the detached element, ready to be captured. */
  function buildPdfPage(reportNodes) {
    var PAGE_W = 720;                               // matches the dialog width so wrapping is identical
    var PAGE_H = Math.round(PAGE_W * 297 / 210);    // A4 proportions (~1018px)

    var page = document.createElement("div");
    page.className = "plan-pdf-page";
    page.style.cssText =
      "position:fixed;left:-10000px;top:0;width:" + PAGE_W + "px;min-height:" + PAGE_H + "px;" +
      "display:flex;flex-direction:column;background:#fffdf8;box-sizing:border-box;margin:0;z-index:-1;";

    // header — clone the real bar so fonts and colours match exactly
    var bar = document.querySelector(".plan-modal__bar");
    if (bar) {
      var header = bar.cloneNode(true);
      var closeBtn = header.querySelector(".plan-modal__close");
      if (closeBtn && closeBtn.parentNode) closeBtn.parentNode.removeChild(closeBtn);
      var dateEl = header.querySelector(".plan-modal__date");
      if (dateEl) {
    ...
    // body fills the page below the header; cards are centred vertically so a
    // short page (e.g. worlds 3 & 4) stays balanced instead of leaving a gap
    var body = document.createElement("div");
    body.style.cssText =
      "flex:1 1 auto;display:flex;flex-direction:column;justify-content:center;padding:18px 20px;";
    var report = document.createElement("div");
    report.className = "plan-report";
    reportNodes.forEach(function (n) { report.appendChild(n.cloneNode(true)); });
    // no trailing card margin at the foot of a page
    var worldsIn = report.querySelectorAll(".plan-world");
    if (worldsIn.length) worldsIn[worldsIn.length - 1].style.marginBottom = "0";
  ...
  function downloadRoadmapPDF(report) {
    ...
    if (!node || !node.innerHTML.trim()) {
      if (node) node.innerHTML = renderReportHTML(report || currentReport || collectReport());
    }
    var worlds = node ? node.querySelectorAll(".plan-world") : [];
    if (!node || !worlds.length) {
      resetPrintBtn();
      setStatus("Couldn't build the PDF — please try again.", "err");
      return;
    }
    var head = node.querySelector(".plan-report__head");

    // group worlds two per page; the overall-readiness head sits on page one
    var pages = [];
    for (var i = 0; i < worlds.length; i += 2) {
      var g = [worlds[i]];
      if (worlds[i + 1]) g.push(worlds[i + 1]);
      pages.push((i === 0 && head) ? [head].concat(g) : g);
    }

    function fail(e) {
      if (e && window.console && console.error) console.error("roadmap pdf:", e);
      resetPrintBtn();
      setStatus("Couldn't build the PDF — please try again.", "err");
    }
    ...
    fontsReady.then(function () {
      // capture each page group to its own canvas, one at a time
      return pages.reduce(function (chain, nodes) {
        return chain.then(function (canvases) {
          var el = buildPdfPage(nodes);
      ...
      // one shared width for every page keeps the scale and side margins
      // consistent; only shrink if a page would otherwise overflow the height
      var maxAspect = 0;
      canvases.forEach(function (c) { maxAspect = Math.max(maxAspect, c.height / c.width); });
      var drawW = Math.min(PW, PH / maxAspect);

      canvases.forEach(function (canvas, idx) {
        if (idx > 0) pdf.addPage();
        pdf.setFillColor(253, 253, 248);
